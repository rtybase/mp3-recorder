...
enum STEREOMODE {STEREO, JOINTSTEREO, DUALCHANNEL};
...
public:
	...
	CMP3Simple(unsigned int nBitRate, unsigned int nInputSampleRate = 44100,
		unsigned int nOutSampleRate = 0, STEREOMODE eMode = JOINTSTEREO);
...
CMP3Simple::CMP3Simple(unsigned int nBitRate, unsigned int nInputSampleRate,
						   unsigned int nOutSampleRate, STEREOMODE eMode) {
	BE_ERR		err = 0;
	...
	switch (eMode) {
	case STEREO: 
		this->beConfig.format.LHV1.nMode = BE_MP3_MODE_STEREO;
		break;
	case DUALCHANNEL:
		this->beConfig.format.LHV1.nMode = BE_MP3_MODE_DUALCHANNEL;
		break;
	default:
	...
	//if (nBitRate < 32) this->beConfig.format.LHV1.dwMpegVersion = MPEG1;//2;
	//else if (nBitRate == 144) this->beConfig.format.LHV1.dwMpegVersion = MPEG1;//2;
	//else this->beConfig.format.LHV1.dwMpegVersion = MPEG1;

	// Try MPEG2
	this->beConfig.format.LHV1.dwMpegVersion = MPEG2;
	...
	if(err != BE_ERR_SUCCESSFUL) {
		// Another try, but with MPEG1 this time
		this->beConfig.format.LHV1.dwMpegVersion = MPEG1;
		err = beInitStream(&this->beConfig, &this->dwPCMBuffer, &this->dwMP3Buffer, &this->hbeStream);
