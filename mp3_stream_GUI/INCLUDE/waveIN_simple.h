...
#define WAVEHEADERS 32
#define MAXBUFFLEN 5120
...
private:
	...
	WAVEHDR	m_WaveHeader[WAVEHEADERS];
...
void CWaveINSimple::Close(int iLevel) {
	...
	case 1:
	case 2:
		for (i = 0; i < WAVEHEADERS; i++) waveInUnprepareHeader(this->m_WaveInHandle, &this->m_WaveHeader[i], sizeof(WAVEHDR));
	case 3:
...
void CWaveINSimple::_Stop() {
	if (this->m_WaveInHandle != NULL) {
		...
		while (this->m_BuffersDone > 0) ::Sleep(1);
		this->Close(1);

		for (int i = 0; i < WAVEHEADERS; i++) this->m_WaveHeader[i].dwFlags = 0;
	}
}
...
void CWaveINSimple::_Start(IReceiver *pReceiver) {
	...
	if (this->m_WaveInHandle == NULL) {
		...
		if (this->m_WaveHeader[0].lpData == NULL) {
			this->m_WaveHeader[0].lpData = (char *)VirtualAlloc(0, this->m_WaveHeader[0].dwBufferLength * WAVEHEADERS, MEM_COMMIT, PAGE_READWRITE);
			if (this->m_WaveHeader[0].lpData == NULL) {
				this->Close(3);
				throw "Can't allocate memory for WAVE buffer.";
			}
		}
		for (i = 0; i < (WAVEHEADERS - 1); i++) {
			this->m_WaveHeader[i+1].lpData = this->m_WaveHeader[i].lpData + this->m_WaveHeader[i].dwBufferLength;
		}

		for (i = 0; i < WAVEHEADERS; i++) {
			err = waveInPrepareHeader(this->m_WaveInHandle, &this->m_WaveHeader[i], sizeof(WAVEHDR));
			if (err) {
				this->Close(1);
				throw "Error preparing WAVEHDR.";
			}
		}

		for (i = 0; i < WAVEHEADERS; i++) {
			err = waveInAddBuffer(this->m_WaveInHandle, &this->m_WaveHeader[i], sizeof(WAVEHDR));
			if (err) {
				this->Stop();
				throw "Error queueing WAVEHDR.";
			}
			this->m_BuffersDone++;
		}
...
CWaveINSimple::CWaveINSimple(UINT nWaveDeviceID, WAVEINCAPS *pWIC): m_Mixer(nWaveDeviceID), m_qLocalMutex() {
	...
	ZeroMemory(this->m_WaveHeader, sizeof(WAVEHDR) * WAVEHEADERS);
	for (int i = 0; i < WAVEHEADERS; i++) {
		this->m_WaveHeader[i].dwBufferLength = MAXBUFFLEN;//(this->m_waveFormat.nAvgBytesPerSec / WAVEHEADERS) + 1;
	}
}
